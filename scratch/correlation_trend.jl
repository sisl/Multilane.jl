...
point = TestSet(rmodel_type="TargetLaneReward", vel_sigma=0.25, p_appear=1.0, brake_threshold=4.0, brake_terminate_thresh=4.0, N=500)

tests = []
for cor in cors
    bg = @sprintf("cor_%03d", 100*cor)
    push!(tests, TestSet(point, solver_key="dpw", key=@sprintf("upper_bound_%03d", 100*cor), behaviors=bg))
    push!(tests, TestSet(point, solver_key="assume_normal", key=@sprintf("assume_normal_%03d", 100*cor), behaviors=bg))
    push!(tests, TestSet(point, solver_key="pomcp", key=@sprintf("pomcp_%03d", 100*cor), behaviors=bg))
    push!(tests, TestSet(point, solver_key="mlmpc", key=@sprintf("mlmpc_%03d", 100*cor), behaviors=bg))
end

objects = gen_initials(tests, behaviors=behaviors, generate_physical=true)

println(objects["param_table"])
