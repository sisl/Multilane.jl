...
function test_iteration()
    ...
    s = MLState(false, CarState[CarState(50.0, 2.0, 30.0, 0.0, Multilane.NORMAL, 0)])
    as = actions(mdp)
    as = actions(mdp, s, as)
...
function test_off_road()
    ...
    mdp = actions_mdp_fixture()
    as = actions(mdp)
    right = MLState(false, CarState[CarState(50.0, 1.0, 30.0, 0.0, Multilane.NORMAL, 0)])
    ar = collect(actions(mdp, right, as))
    @test length(ar) > 1
    @test all(a -> a.lane_change >= 0., ar)
    left = MLState(false, CarState[CarState(50.0, 3.0, 30.0, 0.0, Multilane.NORMAL, 0)])
...
function test_all_safe()
    ...
    mdp = actions_mdp_fixture()
    as = actions(mdp)
    allsafe = MLState(false, CarState[CarState(50.0, 2.0, 30.0, 0.0, Multilane.NORMAL, 0)])
...
function failure_4()
    ...
    s = MLState(false, CarState[CarState(50.0,3.0,31.22970781020354,0.0,Multilane.NORMAL,1),CarState(99.24797069990433,4.0,27.167643727404155,0.0,Multilane.IDMMOBILBehavior(Multilane.IDMParam(2.0,2.0,0.8,27.5,4.0,4.0),Multilane.MOBILParam(0.0,4.0,0.2),3),2)])
