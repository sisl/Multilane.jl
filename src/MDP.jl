...
function reward(mdp::OriginalMDP,s::MLState,a::MLAction,sp::MLState)
	if is_crash(mdp,s,a) || isterminal(mdp,sp)
		return mdp.rmodel.r_crash
	end
	return __reward(mdp,s,a)
end
...
isterminal(mdp::MLMDP,s::MLState,a::MLAction=MLAction()) = is_crash(mdp,s,a) #placeholder
...
create_state(p::OriginalMDP) = MLState(1,p.dmodel.phys_param.v_med,CarState[CarState((-1.,1,),1.,0,p.dmodel.BEHAVIORS[1]) for _ = 1:p.dmodel.nb_cars])
